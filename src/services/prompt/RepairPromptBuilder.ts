...
const copy = {
  ja: {
    ...
    workshop: '操作時の機器・LED設定',
    invalid: '設定が未設定または不正です。次の項目を利用者が修正するまで、不足値を推測せず、教材に適合した修正版として確定しないでください。',
    mismatch: '選択教材と取得した機器の種類が一致しません。利用者が機種と配線を確認するまでGPIOに依存する修正版を確定しないでください。',
    constraints: '修正条件',
    ...
    blocked: '教材設定が不正または未完成、または機器が不一致のため、設定に依存する修正版の生成は保留し、利用者が確認する項目を示す',
  ...
  en: {
    ...
    execution: 'Execution: write and run main.py through Raw REPL using the browser Web Serial API.\nDo not infer the user-entered target UIFlow2 version from the retrieved MicroPython version.',
    ...
    unknownSource: 'main.py associated with this error\nThe main.py executed on the device has not been retrieved. Do not treat the current editor as the failing program. Its relationship with the selected configuration is unconfirmed; device settings are browser-side reference information.',
    workshop: 'Device and LED settings at the operation',
    invalid: 'Settings are missing or invalid. Do not guess values or finalize a kit-compatible repair until the user resolves the following settings.',
    mismatch: 'The selected kit and detected board do not match. Do not finalize GPIO-dependent repairs until the user verifies the board and wiring.',
    ...
    blocked: 'Defer setting-dependent corrected code until the user resolves invalid settings or board mismatch; list what needs confirmation.',
  ...
  zh: {
    ...
    workshop: '操作时的设备与 LED 设置',
    invalid: '设置缺失或无效。在用户修正以下设置之前，不要猜测缺失值，也不要确定为适用该套件的修复代码。',
    mismatch: '所选套件与读取到的设备型号不一致。在用户确认设备和接线之前，不要确定依赖 GPIO 的修复代码。',
    constraints: '修复条件',
    ...
    blocked: '在用户解决无效设置或设备不一致之前，暂缓依赖设置的修复代码，列出需要确认的项目。',
