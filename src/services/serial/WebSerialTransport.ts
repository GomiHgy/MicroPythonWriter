...
export class WebSerialTransport {
  ...
  private readonly onDisconnect = (event: Event & { port?: SerialPort }) => { if (!event.port || event.port === this.port) { this.disconnected = true; this.queue.close(new SerialDisconnectedError()) } }
  ...
  async open(baudRate: number) {
    ...
    this.disconnected = false; await this.port.open({ baudRate, bufferSize: 65536 }); this.startReader()
  }
