...
export function Terminal({ log, dark, autoScroll }: { log: string; dark: boolean; autoScroll: boolean }) {
  ...
  useEffect(() => { if (!host.current) return; const terminal = new Xterm({ convertEol: true, fontFamily: 'ui-monospace, Consolas, monospace', theme: dark ? { background: '#10161f', foreground: '#dce6f2' } : { background: '#fff', foreground: '#17212b' } }); const fit = new FitAddon(); terminal.loadAddon(fit); terminal.open(host.current); fit.fit(); term.current = terminal; const observer = new ResizeObserver(() => fit.fit()); observer.observe(host.current); return () => { observer.disconnect(); terminal.dispose() } }, [dark])
