...
export function AiPreparationPanel({ preparation, onOpenProgram }: Props) {
  ...
  return <div className="ai-preparation">
    <section className="panel ai-start">
      <div className="section-heading"><div><p className="eyebrow">{t('はじめてでも大丈夫')}</p><h2>{t('好きなAIと、光り方を考えよう')}</h2><p>{t('キットを選んで準備文をコピー。AIへ1回貼り付ければ、相談を始められます。')}</p></div><span className="ai-sparkle" aria-hidden="true">✦</span></div>
      <label className="ai-kit-label" htmlFor="ai-kit">{t('使うキット')}</label>
      ...
      <ol className="ai-simple-steps"><li><strong>{t('準備文をコピー')}</strong><span>{t('キットの設定は自動で入ります')}</span></li><li><strong>{t('好きなAIへ貼って送信')}</strong><span>{t('新しい会話で、質問に答えよう')}</span></li><li><strong>{t('コードを貼って「実行」')}</strong><span>{t('できた main.py をプログラム画面へ')}</span></li></ol>
      {preparation.hasPendingChanges && <p className="notice warn">{t('講師用設定に未適用の変更があります。先に「設定を適用」を押してください。適用するまでコピー・ファイル保存はできません。')}</p>}
      ...
      <div className="ai-links" aria-label={t('好きなAIを開く')}><a href="https://chatgpt.com/" target="_blank" rel="noopener noreferrer">{t('{name}を開く ↗', { name: 'ChatGPT' })}</a><a href="https://claude.ai/" target="_blank" rel="noopener noreferrer">{t('{name}を開く ↗', { name: 'Claude' })}</a><a href="https://gemini.google.com/" target="_blank" rel="noopener noreferrer">{t('{name}を開く ↗', { name: 'Gemini' })}</a></div>
...
function TeacherSettings({ preparation }: { preparation: WorkshopPreparation }) {
  ...
  return <details className="advanced-card ai-teacher"><summary><span aria-hidden="true">⚙</span><div><strong>{t('講師用設定')}</strong><small>{t('配布前に確認する設定。この折りたたみは権限制御ではありません。')}</small></div></summary><div className="advanced-body">
    {!draft ? <p>{t('先に「使うキット」で配布用のキットを選んでください。')}</p> : <>
      ...
      <details className="ai-baseline"><summary>{t('基準コードと講師の実機確認')}</summary><p className="ai-help">{t('任意の基準コードです。Bluetoothには対象UIFlow2版で講師が実機確認したコードが必要です。入力・読込だけでは確認済みになりません。コードは実行されません。')}</p>
      ...
      <button className="quiet-button" onClick={() => { if (confirm(t('このキットのブラウザ保存と画面上の変更を削除し、配布時の設定に戻しますか？'))) preparation.resetProfile() }}>{t('配布時の設定に戻す')}</button>
    </>}
  </div></details>
}
