...
import { SerialDisconnectedError, type AppError, type DeviceInfo, type DeviceState } from '../types'
...
export function useProgrammer() {
  ...
  useEffect(() => { const unsubscribe = transport.onData(bytes => setLog(old => `${old}${new TextDecoder().decode(bytes)}`)); const unsubscribeDisconnect = transport.onDisconnectDetected(() => { connectionLost.current = true; device.current = undefined; setLog(old => `${old}\n[USB切断を検出しました。再接続してください。]\n`); force('connection-lost') }); return () => { unsubscribe(); unsubscribeDisconnect(); transport.dispose() } }, [transport])
  const trap = async (stage: string, action: () => Promise<void>) => { try { setError(undefined); await action() } catch (caught) { const disconnected = connectionLost.current || caught instanceof SerialDisconnectedError; force(disconnected ? 'connection-lost' : 'error'); const parsed = parser.parse(caught instanceof Error ? caught.message : String(caught), source) ?? { exceptionType: caught instanceof Error ? caught.name : 'Error', message: caught instanceof Error ? caught.message : String(caught), traceback: caught instanceof Error ? caught.stack ?? caught.message : String(caught), intentionalInterrupt: false }; setError({ ...parsed, stage, repairPrompt: prompt.build(parsed, source, info, log.slice(-8000), stage) }) } }
  const connect = () => trap('USB接続', async () => { connectionLost.current = false; move('requesting-port'); move('opening'); await transport.connect(baudRate); device.current = new MicroPythonDevice(transport); move('connected'); await developmentMode() })
  const reconnect = async () => {
    try {
      connectionLost.current = false; setError(undefined); move('reconnecting'); await transport.reconnect(baudRate); device.current = new MicroPythonDevice(transport); move('connected'); await developmentMode()
    } catch (caught) {
      device.current = undefined; force('connection-lost'); const message = caught instanceof Error ? caught.message : String(caught); const parsed = parser.parse(message, source) ?? { exceptionType: caught instanceof Error ? caught.name : '再接続エラー', message, traceback: message, intentionalInterrupt: false }; setError({ ...parsed, stage: 'USB再接続', repairPrompt: prompt.build(parsed, source, info, log.slice(-8000), 'USB再接続') })
    }
  }
  ...
  const disconnect = async () => { await transport.disconnect(); connectionLost.current = false; device.current = undefined; setInfo(emptyInfo); force('disconnected') }
  ...
  return { supported: transport.supported, state, info, log, setLog, error, source, setSource, baudRate, setBaudRate, connect, reconnect, disconnect, developmentMode, load, write, run, stop, setBoot, reset }
}
