...
interface OperationSnapshot { source: string; sourceKnown: boolean; device: DeviceInfo; workshop: WorkshopContext | null }
...
export function useProgrammer(workshop: WorkshopContext | null = null) {
  ...
  const capture = (sourceKnown = true): OperationSnapshot => ({ source: sourceKnown ? source : '', sourceKnown, device: structuredClone(infoRef.current), workshop: workshop ? structuredClone(workshop) : null })
  const deviceContext = () => activeSnapshot.current ?? fileSnapshot.current ?? capture(false)
  const saveError = (stage: string, parsed: ParsedTraceback, snapshot: OperationSnapshot) => setError({ ...parsed, stage, sourceSnapshot: snapshot.sourceKnown ? snapshot.source : undefined, sourceKnown: snapshot.sourceKnown, deviceSnapshot: snapshot.device, repairPrompt: prompt.build(parsed, snapshot.source, snapshot.device, logRef.current.slice(-8000), stage, snapshot.workshop, { sourceKnown: snapshot.sourceKnown }) })
  ...
  const connect = () => trap('USB接続', async () => { connectionLost.current = false; move('requesting-port'); move('opening'); await transport.connect(baudRate); device.current = new MicroPythonDevice(transport); move('connected'); await normalMode() }, undefined, capture(false))
  const reconnect = async () => { const snapshot = capture(false); try { connectionLost.current = false; setError(undefined); move('reconnecting'); await transport.reconnect(baudRate); device.current = new MicroPythonDevice(transport); move('connected'); await normalMode() } catch (caught) { connectionLost.current = true; device.current = undefined; force('connection-lost'); showError('SERIAL_DISCONNECTED', caught, snapshot) } }
  ...
  const load = () => {
    ...
    return trap('プログラム読込み', async () => { if (!device.current || programOperation.current !== undefined) return; const next = await device.current.files.readMain(); fileSnapshot.current = { ...snapshot, source: next, sourceKnown: true }; if (source && source !== starter && !confirm('ローカルの未保存編集を上書きしますか？')) return; setSource(next) }, undefined, deviceContext())
  }
  ...
  const reset = () => trap('ハードリセット', async () => { if (!device.current || programOperation.current !== undefined || !confirm('MicroPython機器をリセットします。続ける？')) return; await device.current.prepareForWrite(); move('resetting'); await device.current.boot.reset(); await finishReset() }, undefined, deviceContext())
