...
beforeEach(() => {
  ...
  vi.spyOn(FileTransferService.prototype, 'takePreparedProgram').mockReturnValue({ token: 'test-run', path: '/flash/main.py', byteLength: 100, digest: 'test-digest' })
...
describe('修正依頼のコードと教材のスナップショット', () => {
  ...
  it('ローカル保存が利用不可でも編集・実行を継続できる', async () => {
    ...
    expect(FileTransferService.prototype.writeMain).toHaveBeenLastCalledWith('print("memory-only")', true, true)
  })
})
...
describe('実行・編集・再実行（実際のhook・状態機械、機器通信は模擬）', () => {
  it.each(['still-running', 'startup-marker'] as const)('%s: 手動停止なしで編集内容を再実行する', async confirmation => {
    ...
    expect(FileTransferService.prototype.writeMain).toHaveBeenLastCalledWith('print("edited")', true, true)
  ...
  it.each(['still-running', 'startup-marker'] as const)('%s: 更新は自動停止後に保存だけ行う', async confirmation => {
    ...
    expect(FileTransferService.prototype.writeMain).toHaveBeenLastCalledWith('print("saved")', true, false)
    expect(RawReplClient.prototype.startLongRunning).toHaveBeenCalledTimes(1)
    expect(render().error).toBeUndefined()
  })
