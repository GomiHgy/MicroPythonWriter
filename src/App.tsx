...
const statusCopy: Record<string, { icon: string; eyebrow: string; title: string; description: string; tone: 'ready' | 'running' | 'waiting' | 'warning' | 'error' }> = {
  ...
  error: { icon: '!', eyebrow: '確認が必要', title: 'エラーが見つかりました', description: '操作欄の結果と「困ったとき」で、エラー内容を確認してください。', tone: 'error' },
}
...
export default function App() {
  ...
  return <main className="app">
    ...
    {(connected || app.programFeedback) && <section className="action-card">
      ...
      <div className="main-actions"><button className="run-button" disabled={!canModifyProgram} onClick={runProject}><span>{t("▶ 実行")}</span><small>{t(runDescription)}</small></button><button className="update-button" disabled={!canModifyProgram} onClick={app.write}><span>{t("プログラム更新")}</span><small>{t("保存だけ。今は動かしません。")}</small></button><button className="load-button" disabled={!ready} onClick={app.load}>{t("保存済みのプログラムを読む")}</button></div>
      {!app.programFeedback && <p className="action-tip">{t("迷ったら「実行」。保存だけなら「プログラム更新」を使ってください。")}</p>}
    </section>}
