...
- 無線有効時だけ標準 `bluetooth.BLE` を開始。NanoLED v2、選択したモード、組込み1回アクション `SPARKLE`（約1秒）を提供。演出中の追加ACTIONも受け付け、最後の実出力から200ms補間で再スタートする。最後の受付から約1秒後に、最初の演出前のモード位相・再生状態へ戻る。再押下で復帰元を上書きせず、後で実行する演出キューは作らない。
