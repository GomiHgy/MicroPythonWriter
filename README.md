...
画面タイトルは「AIとフルカラーLED電飾をはじめよう」。キットの既定表示名は機種名のみとし、開催後も利用できる。

「AIの準備」で機器を選ぶと、LED数・最大輝度・外部LEDピンを入力できる。保存先は現在のブラウザで、別ブラウザ・別端末には同期しない。空欄や不正値は保存せず準備文の出力を止め、直前の有効な保存値を残す。保存できない場合は通知する。外部LEDピンは数値範囲内でも利用可能・安全とは限らないため、対象機器の出力可能GPIO、実配線、USB・内蔵LED・ボタンとの競合、電源条件を確認する。

LED設定を変更しても既存のコードを書き換えたり実行したりしない。次に生成する準備・修正依頼に反映し、過去の実行エラーには実行時の設定を使う。LED条件の変更時は基準コードの古い実機確認を失効させる。基準コードやほかの設定を自動保存するものではない。
...
両機種とも **LED数10・最大輝度20%・外部LEDピンGPIO2** が初期値。「AIの準備」で利用者が直接変更でき、有効な3項目だけを機種別にブラウザへ自動保存する。既存の保存済みLED数・最大輝度は維持し、旧形式の外部LEDピンには従来値GPIO2を補う。キットID・UIFlow2版・LED型番は未設定のままで、推測で補完しない。LED_BPP=3、選択機種のボタンピン、GRB、bitstream条件、OFF→点灯時200ms以上は固定仕様。設定が不足・不正なら参加者のコピーとテキスト保存は無効になるが、通常のUSB書込みは止めない。
